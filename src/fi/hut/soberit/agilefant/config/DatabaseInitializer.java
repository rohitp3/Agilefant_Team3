...
public class DatabaseInitializer implements InitializingBean {
	...
	public void afterPropertiesSet() throws Exception {
		...
		// Method changed by Wilfred for SSDI to insert example data into new
		// products, projects and iterations tables
		...
		jdbc.update(
				"INSERT INTO users (fullName, loginName, passwd, initials, enabled, recentItemsNumberOfWeeks) VALUES (?, ?, ?, ?, ?, ?)",
				"Administrator", "admin", "$2a$10$fkULKc1/AmruYR9HTmh5ROc5692D3WaPidZ7dHz073W33AvU2Vqom", "Admin", 1,
				16);
		jdbc.update(
				"INSERT INTO users (admin, fullName, loginName, passwd, initials, enabled, recentItemsNumberOfWeeks) VALUES (?, ?, ?, ?, ?, ?, ?)",
				0, "readonly", "readonly", "$2a$10$fkULKc1/AmruYR9HTmh5ROc5692D3WaPidZ7dHz073W33AvU2Vqom", "readonly",
				1, 0);
		...
		try {
			...
			jdbc.update(
					"INSERT INTO stories (backlog_id, description, iteration_id, name, parent_id, state, storyPoints, storyValue, treeRank) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
					projectId,
					"This is an example story.<br><br>You can create new stories from the upper left \"Create new\" menu or from \"Create story\" button in story list and story tree views.",
					iterationId, "Example Story", null, 0, null, null, 0);
			...
			jdbc.update("INSERT INTO storyrank (backlog_id, rank, story_id) VALUES (?, ?, ?)", projectId, 0, storyId);

			jdbc.update("INSERT INTO storyrank (backlog_id, rank, story_id) VALUES (?, ?, ?)", iterationId, 0, storyId);

			jdbc.update("INSERT INTO story_user (Story_id, User_id) VALUES (?, ?)", storyId, userId);

			log.info("Initialized stories table with example story");

			jdbc.update("INSERT INTO teams (description, name) VALUES (?, ?)", "Example team for admin user",
					"Admin Team");

			int teamId = jdbc.queryForInt("SELECT id FROM teams WHERE name='Admin Team'");

			jdbc.update("INSERT INTO team_product (Team_id, Product_id) VALUES (?, ?)", teamId, productId);

			jdbc.update("INSERT INTO team_user (Team_id, User_id) VALUES (?, ?)", teamId, userId);
