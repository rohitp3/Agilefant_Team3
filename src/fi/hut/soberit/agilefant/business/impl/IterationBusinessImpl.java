...
import fi.hut.soberit.agilefant.model.IterationEntity;
...
@Transactional
public class IterationBusinessImpl extends GenericBusinessImpl<Iteration> implements IterationBusiness {
	...
	public void delete(Iteration iteration) {
		...
		// Method changed by Wilfred for SSDI to delete the Iteration info from
		// new "iterations" table
		...
		for (Story item : stories) {
			...
			/*
			 * In case of normal iteration, story is removed from iteration and
			 * also from it's parent project.
			 */
		...
		IterationEntity iterationEntity = (IterationEntity) session.get(IterationEntity.class, iteration.getId());
		if (iterationEntity != null) {
			session.delete(iterationEntity);
		}
	...
	public IterationTO getIterationContents(int iterationId) {
		...
		Map<Integer, Long> taskEsData = this.iterationDAO.calculateIterationTaskEffortSpent(iteration);
	...
	public ExactEstimate calculateDailyVelocity(Iteration iteration) {
		final LocalDate today = new LocalDate();
		final IterationHistoryEntry entry = iterationHistoryEntryDAO.retrieveByDate(iteration.getId(),
				today.minusDays(1));
		return calculateDailyVelocity(new LocalDate(iteration.getStartDate()), new LocalDate(iteration.getEndDate()),
				entry);
	}
	...
	public IterationMetrics getIterationMetrics(Iteration iteration) {
		...
		IterationHistoryEntry latestHistoryEntry = iterationHistoryEntryBusiness.retrieveLatest(iteration);
		...
		metrics.setDailyVelocity(calculateDailyStoryPointsVelocity(new LocalDate(iteration.getStartDate()),
				new LocalDate(iteration.getEndDate()), doneStoryPoints));
		...
		Pair<Integer, Integer> pairTasks = iterationDAO.getCountOfDoneAndNonDeferred(iteration);
		...
		metrics.setPercentSpentEffort(
				calculatePercent(metrics.getSpentEffort().intValue(), metrics.getPlannedSize().intValue()));
		...
		if (metrics.getEffortLeft() != null && metrics.getOriginalEstimate() != null) {
			...
			metrics.setCompletedEffortPercentage(
					calculatePercent(effortDone, metrics.getOriginalEstimate().intValue()));
		}
	...
	@Override
	public IterationTO store(int iterationId, int parentBacklogId, Iteration iterationData, Set<Integer> assigneeIds,
			Set<Integer> teamIds) {
		// Method changed by Wilfred for SSDI to store the Iteration info in new
		// "iterations" table
		...
		if (iterationId == 0) {
			IterationTO stored = transferObjectBusiness
					.constructIterationTO(this.create(parent, iterationData, assigneeIds, teamIds));

			IterationEntity iterationEntity = new IterationEntity();
			iterationEntity.setId(stored.getId());
			iterationEntity.setName(stored.getName());
			iterationEntity.setProjectId(stored.getParent().getId());
			iterationEntity.setDescription(stored.getDescription());
			iterationEntity.setStartDate(stored.getStartDate().toDate());
			iterationEntity.setEndDate(stored.getEndDate().toDate());
			iterationEntity.setBaselineLoad(stored.getBaselineLoad().longValue());
			iterationEntity.setBacklogSize(stored.getBacklogSize().longValue());

			session.persist(iterationEntity);
	...
	public Set<AssignmentTO> calculateAssignedLoadPerAssignee(Iteration iter) {
		List<Task> iterationTasks = this.iterationDAO.getAllTasksForIteration(iter);
		...
		for (Task task : iterationTasks) {
			...
			long taskEffort = (task.getEffortLeft() != null) ? task.getEffortLeft().longValue() : 0;
			if (responsibleCount != 0) {
				Collection<User> responsibles = task.getResponsibles();
				divideTaskLoad(assignments, responsibleCount, taskEffort, responsibles);
			} else if (task.getStory() != null && !task.getStory().getResponsibles().isEmpty()) {
				responsibleCount = task.getStory().getResponsibles().size();
				Collection<User> responsibles = task.getStory().getResponsibles();
				divideTaskLoad(assignments, responsibleCount, taskEffort, responsibles);
		...
		float timeframeLeft = backlogBusiness.calculateBacklogTimeframePercentageLeft(iter);
		float weeksLeft = backlogBusiness.daysLeftInSchedulableBacklog(iter).getDays() / 7.0f;
		...
		for (AssignmentTO assignment : assignments.values()) {
			...
			assignment.setUnassignedLoad(new ExactEstimate((long) (assignedPortion * unassignedLoad)));

			ExactEstimate iterationWorkHours;
			if (iter.getBacklogSize() != null) {
				iterationWorkHours = new ExactEstimate(
						Math.round(timeframeLeft * iter.getBacklogSize().floatValue() * assignedPortion));
			} else {
			...
			if (assignment.getPersonalLoad() != null) {
				assignment.getBaselineLoad().add((long) (weeksLeft * assignment.getPersonalLoad().floatValue()));
			}
			...
			if (iterationWorkHours.longValue() > 0l) {
				assignment.setLoadPercentage(Math.round(100f * assignment.getTotalLoad().floatValue()
	...
	private void divideTaskLoad(Map<Integer, AssignmentTO> assignments, int responsibleCount, long taskEffort,
			Collection<User> responsibles) {
	...
	public Integer calculateVariance(Iteration iter) {
		IterationHistoryEntry latestHistoryEntry = iterationHistoryEntryBusiness.retrieveLatest(iter);
		if (latestHistoryEntry == null) {
			return null;
		}
	...
	public List<AgilefantHistoryEntry> retrieveChangesInIterationStories(Iteration iteration) {
		...
		for (AgilefantHistoryEntry entry : deleted) {
			Story story = this.storyHistoryDAO.retrieveClosestRevision(entry.getObjectId(),
					entry.getRevision().getId());
			entry.setObject(story);
		}

		List<AgilefantHistoryEntry> modified = this.backlogHistoryDAO.retrieveModifiedStories(iteration);
	 ...
	 *
	 * Interface needed: TaskHistoryDAO in db.history package --> add function
	 * retrieveAllTaskRevisions(iteration) Implementation: TaskHistoryDAOImpl in
	 * db.history.impl package --> implement the added function. --> Note that
	 * this is the same as BacklogHistoryDAO, except that it uses the Iteration
	 * class instead of the Backlog class.
	 *
	 * @author aborici
	 *
	 */
	@Override
	@SuppressWarnings("unchecked")
	public List<AgilefantHistoryEntry> retrieveChangesInIterationTasks(Iteration iteration) {

		List<AgilefantHistoryEntry> allTasks = this.taskHistoryDAO.retrieveAllTaskRevisions(iteration);
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<AgilefantHistoryEntry> renderSortedTaskAndStoryRevisions(Iteration iteration) {
